...
pub const Camera = struct {
    ...
    pub fn transMat(self: Camera) math.Mat32 {
        ...
        const half_w = @as(f32, @floatFromInt(size.w)) * 0.5;
        const half_h = @as(f32, @floatFromInt(size.h)) * 0.5;
    ...
    pub fn screenToWorld(self: Camera, pos: math.Vec2) math.Vec2 {
        ...
        return inv_trans_mat.transformVec2(.{ .x = pos.x, .y = @as(f32, @floatFromInt(gk.window.height())) - pos.y });
    }
};
