...
pub const OptionsUi = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, data: OptionsUi) !*OptionsUi {
        ...
        const main_cont = try ui.DisplayContainer.create(allocator, .{
            .x = 0,
            .y = 0,
        });

        const buttons_cont: *ui.DisplayContainer = try ui.DisplayContainer.create(allocator, .{
            .x = 0,
            .y = buttons_y,
        });

        const tabs_cont: *ui.DisplayContainer = try ui.DisplayContainer.create(allocator, .{
            .x = 0,
            .y = 50,
        });

        const gen_cont: *ui.DisplayContainer = try ui.DisplayContainer.create(allocator, .{
            .x = 0,
            .y = 100,
            .visible = screen.selected_tab == Tabs.general,
        });

        const keys_cont: *ui.DisplayContainer = try ui.DisplayContainer.create(allocator, .{
            .x = 0,
            .y = 100,
        ...
        const graphics_cont: *ui.DisplayContainer = try ui.DisplayContainer.create(allocator, .{
            .x = 0,
            .y = 100,
            .visible = screen.selected_tab == Tabs.graphics,
        });
        const perf_cont: *ui.DisplayContainer = try ui.DisplayContainer.create(allocator, .{
            .x = 0,
            .y = 100,
        ...
        const bg_image = try main_cont.createElement(ui.Image, .{ .x = 0, .y = 0, .image_data = .{
            .nine_slice = NineSlice.fromAtlasData(options_background, width, height, 0, 0, 8, 8, 1.0),
        } });
        bg_image.x = 0;

        const buttons_bg_image = try buttons_cont.createElement(ui.Image, .{ .x = 0, .y = -20, .image_data = .{
            .nine_slice = NineSlice.fromAtlasData(text_input_press, width, button_height + 40, 8, 8, 32, 32, 1.0),
        } });
        buttons_bg_image.x = 0;

        const cls_but = try buttons_cont.createElement(ui.Button, .{
        ...
        const disc_but = try buttons_cont.createElement(ui.Button, .{
        ...
        const reset_but = try buttons_cont.createElement(ui.Button, .{
            .x = 50,
            .y = buttons_bg_image.y + button_half_height,
            .image_data = .{
                .base = .{ .nine_slice = NineSlice.fromAtlasData(button_data_base, button_width, button_height, 6, 6, 7, 7, 1.0) },
        ...
        const title_text = try main_cont.createElement(ui.UiText, .{ .x = buttons_x - 76, .y = 50, .text_data = .{
        ...
        const gen_tab = try tabs_cont.createElement(ui.Button, .{
            .x = tab_x_offset,
            .y = tab_y,
            .image_data = .{
                .base = .{ .nine_slice = NineSlice.fromAtlasData(button_data_base, button_width, button_height, 6, 6, 7, 7, 1.0) },
        ...
        const keys_tab = try tabs_cont.createElement(ui.Button, .{
            .x = tab_x_offset,
            .y = tab_y,
            .image_data = .{
                .base = .{ .nine_slice = NineSlice.fromAtlasData(button_data_base, button_width, button_height, 6, 6, 7, 7, 1.0) },
        ...
        const graphics_tab = try tabs_cont.createElement(ui.Button, .{
            .x = tab_x_offset,
            .y = tab_y,
        ...
        const perf_tab = try tabs_cont.createElement(ui.Button, .{
            .x = tab_x_offset,
            .y = tab_y,
            .image_data = .{
                .base = .{ .nine_slice = NineSlice.fromAtlasData(button_data_base, button_width, button_height, 6, 6, 7, 7, 1.0) },
        ...
        const move_up_map = try gen_cont.createElement(ui.KeyMapper, .{
            .x = key_width + magic,
            .y = key_y,
            .image_data = .{
        ...
        const move_down_map = try gen_cont.createElement(ui.KeyMapper, .{
            .x = key_width + magic,
            .y = key_y,
            .image_data = .{
                .base = .{ .nine_slice = NineSlice.fromAtlasData(button_data_base, key_width, button_height, 6, 6, 7, 7, 1.0) },
        ...
        const move_right_map = try gen_cont.createElement(ui.KeyMapper, .{
            .x = key_width + magic,
            .y = key_y,
            .image_data = .{
                .base = .{ .nine_slice = NineSlice.fromAtlasData(button_data_base, key_width, button_height, 6, 6, 7, 7, 1.0) },
        ...
        const move_left_map = try gen_cont.createElement(ui.KeyMapper, .{
            .x = key_width + magic,
            .y = key_y,
            .image_data = .{
                .base = .{ .nine_slice = NineSlice.fromAtlasData(button_data_base, key_width, button_height, 6, 6, 7, 7, 1.0) },
        ...
        const vsync_tog = try graphics_cont.createElement(ui.Toggle, .{
        ...
        vsync_tog.y = toggle_y;

        toggle_y += toggle_y_offset;

        const lights_tog = try graphics_cont.createElement(ui.Toggle, .{
            .x = toggle_x_offset + magic,
            .y = toggle_y,
            .off_image_data = .{
                .base = .{ .nine_slice = NineSlice.fromAtlasData(toggle_data_base_off, toggle_width, toggle_height, 0, 0, 84, 48, 1.0) },
        ...
        lights_tog.y = toggle_y;

        toggle_y += toggle_y_offset;

        const glow_tog = try graphics_cont.createElement(ui.Toggle, .{
            .x = toggle_x_offset + magic,
            .y = toggle_y,
            .off_image_data = .{
                .base = .{ .nine_slice = NineSlice.fromAtlasData(toggle_data_base_off, toggle_width, toggle_height, 0, 0, 84, 48, 1.0) },
    ...
    pub fn deinit(self: *OptionsUi) void {
        ...
        //self.gen_cont = undefined;
        //self.keys_cont = undefined;
        //self.perf_cont = undefined;
        //self.graphics_cont = undefined;
