...
@group(0) @binding(3) var medium_tex: texture_2d<f32>;
@group(0) @binding(4) var medium_italic_tex: texture_2d<f32>;
@group(0) @binding(5) var bold_tex: texture_2d<f32>;
@group(0) @binding(6) var bold_italic_tex: texture_2d<f32>;
...
const normal_render_type = 0.0;
const text_render_type = 1.0;
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    if in.render_type == normal_render_type {
        let pixel = textureSampleGrad(atlas_tex, default_sampler, in.uv, dx, dy);
    ...
    if in.text_type == medium_text_type {
        ...
        tex_offset = textureSampleGrad(medium_tex, linear_sampler, in.uv - in.shadow_texel_offset, dx, dy);
        ...
        tex_offset = textureSampleGrad(medium_italic_tex, linear_sampler, in.uv - in.shadow_texel_offset, dx, dy);
        ...
        tex_offset = textureSampleGrad(bold_tex, linear_sampler, in.uv - in.shadow_texel_offset, dx, dy);
        ...
        tex_offset = textureSampleGrad(bold_italic_tex, linear_sampler, in.uv - in.shadow_texel_offset, dx, dy);
    }
