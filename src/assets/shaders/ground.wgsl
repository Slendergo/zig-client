...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
  ...
  if in.left_blend_uv.x >= 0.0 && textureSampleGrad(tex, default_sampler, uniforms.leftTopMaskUv.xy + in.uv, dx, dy).a == 1.0 {
    return textureSampleGrad(tex, default_sampler, in.left_blend_uv + in.uv, dx, dy);
  }

  if in.top_blend_uv.x >= 0.0 && textureSampleGrad(tex, default_sampler, uniforms.leftTopMaskUv.zw + in.uv, dx, dy).a == 1.0 {
    return textureSampleGrad(tex, default_sampler, in.top_blend_uv + in.uv, dx, dy);
  }

  if in.right_blend_uv.x >= 0.0 && textureSampleGrad(tex, default_sampler, uniforms.rightBottomMaskUv.xy + in.uv, dx, dy).a == 1.0 {
    return textureSampleGrad(tex, default_sampler, in.right_blend_uv + in.uv, dx, dy);
  }

  if in.bottom_blend_uv.x >= 0.0 && textureSampleGrad(tex, default_sampler, uniforms.rightBottomMaskUv.zw + in.uv, dx, dy).a == 1.0 {
    return textureSampleGrad(tex, default_sampler, in.bottom_blend_uv + in.uv, dx, dy);
  }

  return textureSampleGrad(tex, default_sampler, in.uv + in.base_uv, dx, dy);
}